        ...
        public class UdpFileSender
        {
            ...
            public void Init()
            {
                ...
                if (_transferableFiles.Count != 0)
                {
                    ...
                    Console.WriteLine("Danh sách file có thể gửi:");
                    foreach (string s in _transferableFiles)
                        Console.WriteLine("{0}. {1}", _transferableFiles.ToList().IndexOf(s) + 1, s);
                }
                else
            ...
            {
                if (!Running) return; // Nếu không chạy thì không kiểm tra
